...
const RecipeItem: FC<RecipeHit> = ({_links, recipe}) => {
  ...
  return (
    <Link
      ...
      className="flex relative items-center justify-center flex-col h-auto w-auto group transition delay-150 lg:min-h-80 min-h-32 "
    >
      <img
        src={recipe.image}
        alt={recipe.label}
        className="rounded-md w-full hover:blur-sm duration-300 transition-all relative"
      />
